export type RenderOptions = Readonly<{ label: string; value: number; displayValue?: string | undefined; unit: string; decimalPlaces: number; warningThreshold?: number | undefined; criticalThreshold?: number | undefined }>;
...
export function renderMetric(options: RenderOptions, state: RenderState): string {
  if (state !== "online") return `<svg xmlns="http://www.w3.org/2000/svg" width="144" height="144"><rect width="144" height="144" fill="#20242b"/><text x="72" y="72" fill="#ffb020" text-anchor="middle" font-size="16">${escapeXml(state)}</text></svg>`;
